# pyfreepbx — UniqueOS Integration Audit

How UniqueOS consumes `pyfreepbx`, and the public surface pinned by
`tests/test_integration_patterns.py`. pyfreepbx is the **most-used** vendor SDK
in UniqueOS (16 call-site files) and was, before this audit, the least
...
UniqueOS uses pyfreepbx through the `FreePBX` facade and the `parse_event` /
...
| UniqueOS file | pyfreepbx surface used | Purpose |
|---|---|---|
...
## Public surface UniqueOS depends on (pinned by the test)
...
asserts presence of the fields UniqueOS reads, so a future rename fails here
first.
