...
> Integration field notes for the **most-used** vendor SDK in UniqueOS
...
> - **[CONSUMER]** — a guard/quirk learned from how UniqueOS actually calls it.
...
- **[CONTRACT]** `FreePBXConfig.has_oauth2` is `bool(client_id and client_secret)`.
  ...
  static `api_token` Bearer. UniqueOS mirrors this precedence at
...
- **[CONTRACT]** AMI is the **stable** half of the SDK (live events + privileged
  ...
  event. Consumers MUST filter it (`uniqueos/access_control/call_listener.py`).
...
- **[CONSUMER] originate timeout:** `AMIClient.originate(timeout_ms=30000)`
  defaults to **30 s** — too long for an operator panel. UniqueOS pins
...
- **[CONTRACT]** `FreePBX.from_url()` accepts a full URL **or a bare hostname**.
  ...
  `port=`) override URL-derived values. This is the constructor UniqueOS uses
...
See `docs/integration-audit.md` for the full UniqueOS call-site map and the
